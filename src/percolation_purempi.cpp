...
// co_mpi::Task<void> initialize(int rank){
void initialize(){
  ...
  if(rank == 0) {
    ...
    for(int i = 1; i < X*Y; ++i) {
      // co_await co_mpi::Send( &bigmap, M*N, MPI_INT, i, 0, MPI_COMM_WORLD );
      MPI_Ssend( &bigmap, M*N, MPI_INT, i, 0, MPI_COMM_WORLD );
    }
  } else {
    // co_await co_mpi::Recv( &bigmap, M*N, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
...
// co_mpi::Task<void> sync(int rank) {
void sync() {
  MPI_Datatype column;
  MPI_Request r[8];
